...
//estudar essa gambiarra extrema
    private void jbSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbSalvarActionPerformed
        int codigoVenda = 0;
        listaModelVendaProduto = new ArrayList<>();

        modelVenda.setCliente(Integer.parseInt(jtfCodigoCliente.getText()));
        modelVenda.setProduto(Integer.parseInt(jtfCodigoDoProduto.getText()));
        try {
            modelVenda.setDataVenda((Date) converteData.convertToAmericanDate((Date) new java.util.Date()));

        } catch (Exception ex) {
            Logger.getLogger(ViewVendas.class.getName()).log(Level.SEVERE, null, ex);
        }

        modelVenda.setVenValorLiquido(Double.parseDouble(jtfValorTotal.getText()));
        modelVenda.setVenValorBruto(Double.parseDouble(jtfValorTotal.getText()) + Double.parseDouble(jtfDesconto.getText()));
        modelVenda.setVenValorDesconto(Double.parseDouble(jtfDesconto.getText()));
        ...
        if (codigoVenda > 0) {
            ...
            JOptionPane.showMessageDialog(rootPane, "VENDA NÃO REALIZADA!");
        }

        int cont = jtProdutosVenda.getRowCount();
        int cont2 = jtVendas.getRowCount();
        modelVendaProduto = new ModelVendaProduto();
        for (int i = 0; i < cont; i++) {
            modelVendaProduto.setProduto((int) jtProdutosVenda.getValueAt(i, 0));
            for (int j = 0; j < cont2; j++) {
                modelVendaProduto.setVenda((int) jtVendas.getValueAt(i, 0));
            }
            modelVendaProduto.setVenProValor((double) jtProdutosVenda.getValueAt(i, 3));
            modelVendaProduto.setVenProQuantidade(Integer.parseInt(jtProdutosVenda.getValueAt(i, 2).toString()));
        ...
        if (controllerVendaProduto.SalvarVendaProdutoController(listaModelVendaProduto)) {
            JOptionPane.showMessageDialog(rootPane, "Salvo os Produto da venda");
            carregarVendas();
            limparForm();
        } else {
            JOptionPane.showMessageDialog(rootPane, "erro ao salvar Produto da venda");
        }
